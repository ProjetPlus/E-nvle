  ...
  return (
    <main className="flex-1 flex flex-col overflow-hidden bg-background relative">
      ...
      <div ref={areaRef} className="flex-1 overflow-y-auto px-4 md:px-6 pt-6 pb-2 flex flex-col gap-1 scrollbar-thin z-10">
        ...
        {messages.map((msg) => (
          <motion.div key={msg.id} initial={{ opacity: 0, y: 8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.25 }} className={`flex mb-0.5 ${msg.sent ? "justify-end" : "justify-start"}`}>
            {msg.isTyping ? (
              <div className="flex items-center gap-2 py-2">
                <div className="flex items-center gap-1.5 px-3.5 py-2.5 bg-envle-card border border-envle-border rounded-[18px] rounded-bl-[6px]">
                  {[0, 0.2, 0.4].map((d, i) => (
                    <span key={i} className="w-[7px] h-[7px] rounded-full bg-envle-text-muted" style={{ animation: `typing-bounce 1.4s ease infinite ${d}s` }} />
                  ))}
                </div>
              ...
              <div className={`max-w-[80%] md:max-w-[65%] px-3.5 py-2.5 rounded-[18px] text-sm leading-relaxed ${msg.sent ? "rounded-br-[6px] text-foreground" : "bg-envle-card border border-envle-border rounded-bl-[6px]"}`} style={msg.sent ? { background: "linear-gradient(135deg, hsl(var(--envle-vert-dark)), hsl(var(--envle-vert)))" } : undefined}>
                ...
                {msg.isVoice ? (
                  <div className="flex items-center gap-2.5 px-3 py-2">
                    <motion.div whileTap={{ scale: 0.9 }} className="w-8 h-8 rounded-full bg-foreground/15 flex items-center justify-center text-sm cursor-pointer" onClick={() => toast("🎤 Lecture du message vocal")}>▶️</motion.div>
                    <div className="flex items-center gap-0.5 flex-1">
                      {Array.from({ length: 10 }).map((_, i) => (
                        <div key={i} className="w-[3px] rounded-sm bg-foreground/40" style={{ height: `${12 + Math.random() * 20}px`, animation: `wave-anim 1.2s ease infinite ${i * 0.1}s` }} />
                      ))}
                    </div>
        ...
        <div className="px-3 md:px-5 py-2 bg-envle-card border-t border-envle-border z-10 flex gap-2 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {pendingFiles.map((file) => (
            <motion.div key={file.id} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative shrink-0 flex items-center gap-2 bg-foreground/[0.06] border border-envle-border rounded-xl px-3 py-2">
              ...
              <button className="w-5 h-5 rounded-full bg-envle-rouge/80 border-none text-[10px] text-foreground cursor-pointer flex items-center justify-center" onClick={() => removePendingFile(file.id)}>✕</button>
            </motion.div>
          ))}
        </div>
      )}
      ...
      <div className="px-3 md:px-5 py-3 pb-4 bg-envle-card border-t border-envle-border flex items-end gap-2 md:gap-2.5 z-10">
        <div className="flex-1 bg-foreground/[0.06] border border-envle-border rounded-[22px] px-3 md:px-4 py-2.5 flex items-center gap-2 focus-within:border-primary focus-within:shadow-[0_0_0_3px_hsla(142,47%,33%,0.15)]">
          ...
          <textarea className="flex-1 bg-transparent border-none outline-none text-foreground font-body text-sm resize-none max-h-[100px] placeholder:text-envle-text-muted" placeholder="Écrire un message..." rows={1} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKey} />
