...
const BoutiqueModule = ({ onBack }: { onBack: () => void }) => {
          ...
          <AnimatePresence>
            {filtered.map((product, i) => (
              <motion.div
                ...
                className="bg-envle-card border border-envle-border rounded-2xl overflow-hidden group hover:border-primary/30 transition-all"
              >
                <div
                  className="aspect-square relative flex items-center justify-center text-[64px] group-hover:scale-105 transition-transform duration-500"
                  style={{ background: product.bgStyle }}
                >
                  {product.icon}
                  <motion.button
                    whileTap={{ scale: 1.3 }}
                    className="absolute top-3 right-3 w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm border-none flex items-center justify-center text-base cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <motion.button
                      whileTap={{ scale: 0.9 }}
                      className="px-3 py-1.5 rounded-lg border-none text-xs font-semibold cursor-pointer text-primary-foreground"
                      style={{ background: "linear-gradient(135deg, hsl(var(--envle-vert)), hsl(var(--envle-vert-dark)))" }}
                      onClick={() => toast(`🛒 ${product.name} ajouté au panier`)}
                    >
                      + Panier
                    </motion.button>
                  </div>
