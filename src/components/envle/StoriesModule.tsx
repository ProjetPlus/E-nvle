    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-background">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="px-6 py-4 bg-envle-card border-b border-envle-border flex items-center gap-3">
        ...
        <motion.button
          ...
          className="px-4 py-2 rounded-xl border-none font-body text-sm cursor-pointer font-semibold text-primary-foreground transition-all"
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((story, i) => (
              <motion.div
                ...
                whileTap={{ scale: 0.97 }}
                className="bg-envle-card border border-envle-border rounded-2xl overflow-hidden cursor-pointer group hover:border-primary/30 transition-all hover:shadow-[0_8px_30px_hsla(142,47%,33%,0.1)]"
                onClick={() => setViewingStory(story)}
              >
                <div className="aspect-[4/5] relative overflow-hidden">
                  <motion.div
                    className="w-full h-full flex items-center justify-center text-6xl transition-transform duration-700"
                  ...
                  {story.type === "reel" && (
                    <div className="absolute top-3 right-3 bg-black/50 backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-semibold">🎵 Reel</div>
                  )}
                ...
                <div className="p-3 flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: story.avatarStyle }}>{story.avatar}</div>
          ...
          >
            ...
            >
              ...
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="absolute top-6 left-4 right-4 flex items-center gap-2.5 z-10">
                <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold border-2 border-white/30" style={{ background: viewingStory.avatarStyle }}>{viewingStory.avatar}</div>
              ...
              <div className="w-full h-full flex items-center justify-center text-[80px]">{viewingStory.type === "reel" ? "🎬" : viewingStory.type === "video" ? "🎥" : "📸"}</div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 pt-16">
                <p className="text-base font-medium mb-4">{viewingStory.caption}</p>
                <div className="flex items-center gap-3">
                  <input className="flex-1 bg-white/10 border border-white/20 rounded-full px-4 py-2.5 text-sm text-white placeholder:text-white/50 outline-none font-body" placeholder="Répondre à la story..." onClick={(e) => e.stopPropagation()} />
                  <motion.button whileTap={{ scale: 1.3 }} className="text-2xl bg-transparent border-none cursor-pointer">❤️</motion.button>
                  <motion.button whileTap={{ scale: 1.3 }} className="text-2xl bg-transparent border-none cursor-pointer">➤</motion.button>
                </div>
              </motion.div>
            </motion.div>
          </motion.div>
