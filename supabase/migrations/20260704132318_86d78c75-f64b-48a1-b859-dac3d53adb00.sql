...
CREATE TABLE public.profiles (
  ...
  bio TEXT DEFAULT '', location TEXT DEFAULT '', profession TEXT DEFAULT '', country TEXT DEFAULT '',
...
CREATE TABLE public.conversations (
  ...
  name TEXT, is_group BOOLEAN DEFAULT false, avatar_url TEXT,
...
CREATE TABLE public.messages (
  ...
  file_url TEXT, file_name TEXT, file_size BIGINT,
...
CREATE TABLE public.products (
  ...
  currency TEXT DEFAULT 'XOF', category TEXT DEFAULT '',
  images TEXT[] DEFAULT '{}', stock INT DEFAULT 0, is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE public.jobs (
  ...
  title TEXT NOT NULL, company TEXT DEFAULT '', description TEXT DEFAULT '',
  location TEXT DEFAULT '', salary_range TEXT DEFAULT '',
...
CREATE TABLE public.communities (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL, description TEXT DEFAULT '', avatar_url TEXT,
  created_by UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  member_count INT DEFAULT 0, is_public BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE public.notifications (
  ...
  title TEXT NOT NULL, body TEXT DEFAULT '', icon TEXT DEFAULT '🔔',
  is_read BOOLEAN DEFAULT false,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE public.otp_codes (
  ...
  attempts INT DEFAULT 0, is_used BOOLEAN DEFAULT false,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
BEGIN
  SELECT COUNT(*) INTO v_count FROM public.otp_codes
   WHERE email = p_email AND created_at > now() - interval '24 hours' AND is_used = false;
  IF v_count >= 3 THEN
...
BEGIN
  SELECT * INTO v_otp FROM public.otp_codes
   WHERE email = p_email AND code = p_code AND is_used = false
   ORDER BY created_at DESC LIMIT 1;
  IF NOT FOUND THEN RETURN json_build_object('success', false, 'error', 'Code invalide.'); END IF;
  IF v_otp.expires_at < now() THEN RETURN json_build_object('success', false, 'error', 'Code expiré.'); END IF;
  IF v_otp.attempts >= 3 THEN RETURN json_build_object('success', false, 'error', 'Trop de tentatives.'); END IF;
